  ...
  return (
    <div className="hidden sm:block">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        {/* Services - Tablet View with Accordion */}
        <div className="sm:block lg:hidden">
          <h3 className="text-white text-lg font-semibold mb-4">Services</h3>
          <ul className="space-y-3">
            {serviceCategories.map((category) => (
              <li key={category.name} className="border-b border-zinc-800 pb-2">
                <button 
                  onClick={() => toggleCategory(category.name)}
                  className="flex justify-between items-center w-full text-left hover:text-white transition-colors py-1"
                >
                  <span>{category.name}</span>
                  <svg 
                    className={`w-4 h-4 transition-transform ${expandedCategory === category.name ? 'transform rotate-180' : ''}`} 
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24" 
